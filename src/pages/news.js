...
const NewsItem = ({isAdmin, data : {title, description, date,isApproved}}) => <div className="news-item">
<h5>{title}</h5>
<p>{description}</p>
<div className="news-item__info">
    ...
    <div>
        {isApproved ? 'Одобрено ✅' : isAdmin? 'Одобрить?':'❌ Не одобрено'}
    </div>
</div>
</div>
    ...
    return (
        <div className="newspage">
            ...
            <div className="news">
                ...
                filteredNews.map((data) => <NewsItem isAdmin={isAdmin} key={data.id} data={data}/>):
                <h3>Новостей нет!</h3>}
            </div>
        </div>
    );
